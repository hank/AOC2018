        ...
        for r in result:
            ...
            if re.match(r"falls", entry):
                ...
                if m:
                    ...
                    current_id = gid
        sguards = sorted(guards.items(), key=lambda kv: kv[1]['minutes_asleep'], reverse=True)
        best_guard = sguards[0]
        best_guard[1]['minutes'] = sorted(best_guard[1]['minutes'].items(), key=lambda t: t[1], reverse=True)
        print(best_guard)
        print("Final: {}".format(best_guard[1]['minutes'][0][0] * int(best_guard[0])))

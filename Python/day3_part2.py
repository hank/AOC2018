...
def claim_check(clms, c):
    """Returns if claim is non-overlapping with all other claims"""
    c_cid = c[0]
    c_coords = c[1]
    c_dim = c[2]
        ...
        for c in children:
            ...
            for i in range(start_row, start_row+height):
                for j in range(start_col, start_col+width):
                    ...
                    fabric[i][j] += 1
        # Once complete, find non-overlapping claim
        total_count = 0
        for c in children:
            if claim_check(fabric, c):
                print("{} has no overlap".format(c))
                break
        return total_count
